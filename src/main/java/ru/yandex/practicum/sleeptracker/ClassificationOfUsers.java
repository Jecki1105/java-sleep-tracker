...
public class ClassificationOfUsers implements SleepMonitoring {
    ...
    public SleepAnalysisResult sleepAnalysis(List<SleepingSession> sessions) {
        if (sessions.isEmpty()) {
            return new SleepAnalysisResult(ChronotypeUsers.PIGEON, "Пользователь относится к хронотипу: ");
        }

        List<ChronotypeUsers> chronoList = sessions.stream()
                .filter(this::isSleepWithinSameDay)
                .map(this::chronoUserOfNight)
                .toList();

        long countOwl = chronoList.stream().filter(ChronotypeUsers.OWL::equals).count();
        long countLark = chronoList.stream().filter(ChronotypeUsers.LARK::equals).count();
        long countPigeon = chronoList.stream().filter(ChronotypeUsers.PIGEON::equals).count();

        ChronotypeUsers result;
        if (countOwl > countLark && countOwl > countPigeon) {
            result = ChronotypeUsers.OWL;
        } else if (countLark > countOwl && countLark > countPigeon) {
            result = ChronotypeUsers.LARK;
        } else {
            result = ChronotypeUsers.PIGEON;
        }
        return new SleepAnalysisResult(result, "Пользователь относится к хронотипу: ");
    }

    public ChronotypeUsers chronoUserOfNight(SleepingSession session) {
